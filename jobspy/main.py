...
def parse_args():
    ...
    parser.add_argument(
        "--search_term", default="software engineer", help="Search term for jobs"
    )
    parser.add_argument(
        "--google_search_term",
        default="software engineer jobs near San Francisco, CA since yesterday",
        help="Google specific search term",
    )
    parser.add_argument(
        "--location", default="San Francisco, CA", help="Location for job search"
    )
    parser.add_argument(
        "--distance", type=int, default=50, help="Distance in miles (default: 50)"
    )
    parser.add_argument(
        "--job_type",
    ...
    parser.add_argument(
        "--results_wanted", type=int, default=20, help="Number of results wanted"
    )
    parser.add_argument(
        "--easy_apply",
        action="store_true",
    ...
    parser.add_argument(
        "--offset", type=int, default=0, help="Starts the search from an offset"
    )
    parser.add_argument(
        "--hours_old", type=int, default=72, help="How many hours old the jobs can be"
    )
    parser.add_argument(
        "--verbose",
        type=int,
    ...
    parser.add_argument(
        "--country_indeed", default="USA", help="Country for Indeed search"
    )
    parser.add_argument(
        "--is_remote",
    ...
    parser.add_argument(
        "--linkedin_company_ids", help="Comma-separated list of LinkedIn company IDs"
    )
    parser.add_argument(
    ...
    parser.add_argument("--ca_cert", help="Path to CA Certificate file for proxies")
    parser.add_argument(
        "--output",
...
linkedin_company_ids = (
    [int(id) for id in args.linkedin_company_ids.split(",")]
...
if args.output:
    ...
    if args.format == "csv":
        jobs.to_csv(
            sys.stdout, quoting=csv.QUOTE_NONNUMERIC, escapechar="\\", index=False
        )
    elif args.format == "json":
